...
if (!function_exists('build_grid_js')) {
     ...
     * @return	string
     */
    function build_grid_js($grid_id, $url, $colModel, $sortname, $sortorder, $gridParams = NULL, $buttons = NULL, $onsubmit = NULL, $gridColor = NULL, $onSuccess = NULL) {
        //Basic propreties
        ...
        if (!is_null($onSuccess))
            $grid_js .= "onSuccess: function(){" . $onSuccess . "},";
        ...
        if (!is_null($onsubmit))
            $grid_js .= "onSubmit: " . $onsubmit . ",";
        $grid_js .= "onError:function(xhr,ajaxOptions,thrownError){ajaxError(xhr,ajaxOptions,thrownError);},";
        if (!is_null($gridColor))
            $grid_js .= 'onSuccess: rowColoring,';
        $grid_js .= "colModel : [";
        ...
        if (!is_null($gridColor)) {
            ...
            $grid_js .= 'function rowColoring() { ';
            foreach ($gridColor as $colorCondition) {
                $grid_js .= '$(".bDiv tbody tr").each(function(){var cond = $(this).find("td").eq(';
                $grid_js .= $colorCondition['index'] . ').text();';

                foreach ($colorCondition['condition'] as $key => $value) {
                    if ($colorCondition['mode'] == 'row') {
                        $grid_js .= 'if (cond == "' . $key . '"){if($(this).hasClass("erow")) $(this).removeClass("erow");$(this).css("background", "' . $value . '");}';
                    } else {
                        $grid_js .= 'if (cond == "' . $key . '"){$(this).find("td").eq(' . $colorCondition['index'] . ').css("background", "' . $value . '");}';
                    }
                }
